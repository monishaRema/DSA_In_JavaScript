...
//     const commonFrnds = []
//     usersA.forEach(friend => {
//         usersB.forEach(friend2 =>{
//             if(friend.id == friend2.id){
//                 commonFrnds.push(friend2)
//             }
...
//   console.log(commonFrnds.length)
...
const commonFriends = (usersA,usersB) =>{

    const usersAIdList = new Set(usersA.map(f=>f.id))
    const commonfrnd = []
    usersB.forEach(frnd => {
        if(usersAIdList.has(frnd.id)){
            commonfrnd.push(frnd)
        }
        
    });

    console.log(commonfrnd.length)
}

commonFriends(usersA,usersB)
